...
export function useOvershootVideoFile() {
  ...
  const processVideoFile = useCallback(async (videoFile, videoId, videoName) => {
    if (!videoFile) {
    ...
    if (visionRefs.current.has(videoId)) {
      logWithTimestamp(`⚠️ Video ${videoName} is already being processed`, null, "warn");
      return;
    }
    ...
    try {
      ...
      logWithTimestamp(`✅ Config loaded (${nodesConfig.nodes?.length || 0} nodes) for ${videoName}`);
      ...
      const visionConfig = {
        ...
        onResult: async (result) => {
          ...
          if (resultCountRef.current % 5 === 0) {
            if (isJson) {
              logWithTimestamp(
                `📹 [${videoName}] Result #${resultCountRef.current}: ${Object.keys(parsedResult).join(", ")}`
              );
            } else {
              logWithTimestamp(`📹 [${videoName}] Result #${resultCountRef.current}`);
            }
          }
        ...
        onError: (error) => {
          ...
          if (!errorMsg.includes("WebSocket is closed") && !errorMsg.includes("before the connection")) {
            logWithTimestamp(`❌ [${videoName}] SDK error: ${errorMsg}`, null, "error");
          }
      ...
      logWithTimestamp(`❌ [${videoName}] Failed to process: ${err.message}`, null, "error");
  ...
  const stopProcessing = useCallback(async (videoId) => {
    ...
    try {
      ...
      logWithTimestamp(`❌ Error stopping video ${videoId}: ${err.message}`, null, "error");
      visionRefs.current.delete(videoId);
    }
  }, []);
